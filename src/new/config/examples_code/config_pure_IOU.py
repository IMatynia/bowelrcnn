...
from .classification_models import make_classification_model_baseline
from .pattern_models import make_pattern_model_pure_iou
from .dataset_presets import random_dataset_7_2_1
from .dataset_roots import dataset_315x126


def get_pure_iou_in_patt():
    SEED = 42
    set_seeds(42)
    window = (126, 315)
    ...
    return RCNNModelConfig(
        seed=SEED,
        name="RCNN model with pure iou on pattern model",
        version="1.0",
        description="Bowel sound detection model based around the use of region based CNN",
        dataset=data_setup,
        pattern_model=make_pattern_model_pure_iou(SEED, window),
