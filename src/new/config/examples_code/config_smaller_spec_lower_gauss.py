...
from .classification_models import make_classification_model_less_gauss
from .pattern_models import make_pattern_model_less_gauss
from .dataset_presets import random_dataset_7_2_1
from .dataset_roots import dataset_315x126


def get_smaller_spec_lower_gauss_config():
    SEED = 42
    set_seeds(42)
    window = (64, 126)
    ...
    return RCNNModelConfig(
        seed=SEED,
        name="RCNN model with 126x64 spectrogram with 0.15 std on the gauss augment",
        version="1.0",
        description="Bowel sound detection model based around the use of region based CNN",
        dataset=data_setup,
        pattern_model=make_pattern_model_less_gauss(SEED, window),
        classification_model=make_classification_model_less_gauss(SEED, window),
    )
